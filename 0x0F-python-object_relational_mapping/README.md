...
What? Empty?<br>
Yes, it's an [SQL injection](https://en.wikipedia.org/wiki/SQL_injection) to delete all records of a table...<br>
...
6. Write a python file that contains the class definition of a `State` and an instance `Base = declarative_base()`:
    ...
    - Your script should connect to a MySQL server running on `localhost` at port `3306`
    - __WARNING__: all classes who inherit from `Base` __must__ be imported before calling `Base.metadata.create_all(engine)`
...
11. Write a script that adds the `State` object “Louisiana” to the database `hbtn_0e_6_usa`
    ...
    - You must use the module `SQLAlchemy`
    - You must import `State` and `Base` from `model_state` - `from model_state import Base, State`
    - Your script should connect to a MySQL server running on `localhos`t at port `3306`
